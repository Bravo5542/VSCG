    ...
    for epoch in range(args.nb_epoch):
        ...
        if epoch != 0 :
            ...
            if test_acc >= best_test_acc:
                ...
                torch.save(net_model, 'model1/' + model_name + "_" + str(epoch) + "_fully.pt")
    print('[best val accuracy]: ', best_val_acc)
...
def test(args, net_model, model_path=None):
    ...
    net_model.eval()
    AVEData = AVE_Fully_Dataset(video_dir=args.dir_video, audio_dir=args.dir_audio, label_dir=args.dir_labels,
                         order_dir=args.dir_order_test, batch_size=20, status='test')
    nb_batch = AVEData.__len__()
    audio_inputs, video_inputs, labels, _, _, = AVEData.get_batch(2)
    ...
    #compute_accuracy_supervised(is_event_scores, event_scores, labels)
